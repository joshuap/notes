...
ROAM_PAGES.each do |page|
  ...
  while (child = children.pop)
    ...
    uid = child["uid"].to_s.gsub(/[^A-Za-z0-9_-]/, "")
    next if uid.blank?

    ([child] | Array(child["children"])).each do |grandchild|
      guid = grandchild["uid"].to_s.gsub(/[^A-Za-z0-9_-]/, "")
      next if guid.blank?

      grandchild["permalink"] ||= if child["heading"] == 1
        if uid == guid
          "/block/#{uid}"
        else
          "/block/#{uid}#block-#{guid}"
        end
      else
        "#{page["permalink"]}#block-#{guid}"
      end
    end
    ...
    if child["heading"] == 1
      proxy "/block/#{uid}/index.html", "templates/roam-block.html", data: {page: page, block: child}
    end
  end
